...
> ⚠️ **Every variable ko use karne se pehle** suitable data type ke saath **declare** karna zaroori hai.
...
| Rule | Example | Valid? |
|------|---------|--------|
| Can start with letter | `age` | ✅ |
| Can start with `_` | `_marks` | ✅ |
| Can start with `$` | `$total` | ✅ |
| Cannot start with digit | `2name` | ❌ |
| No spaces allowed | `student name` | ❌ |
| No special chars (except `_`, `$`) | `my@var` | ❌ |
| Cannot be a Java keyword | `class` | ❌ |
...
[⬅️ Back to Index](./README.md) | [Next: Number Literals ➡️](./07-number-literals.md)
