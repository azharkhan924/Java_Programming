...
| **Narrowing** (Explicit) | Larger → Smaller | ✅ Yes | ⚠️ Possible |

---

## 1️⃣ Widening / Implicit Type Casting
...
byte b = 50;
int i = b;              // ✅ Implicit widening: byte → int

char c = 'A';
int n = c;              // ✅ Implicit widening: char → int (65)
```
...
- ⚠️ **Usually** data loss nahi hota

### ⚠️ Important Exception — `long → float` Precision Loss
...
## 2️⃣ Narrowing / Explicit Type Casting
...
float f = (float) 10.5;       // ✅ double → float

int i = 300;
byte b = (byte) i;            // ✅ But value wraps around!
...
char x = 'A';
char y = 'B';
int sum = x + y;      // ✅ 65 + 66 = 131 (result is int)
```

### ⚠️ Common Gotcha — Compound Assignment vs Normal

```java
byte x = 10;
x += 20;           // ✅ Valid! (compound assignment has implicit cast)
// Equivalent to: x = (byte)(x + 20);

byte x = 10;
x = x + 20;        // ❌ Error! (x + 20 gives int, can't assign to byte)
x = (byte)(x + 20); // ✅ Explicit cast required
```
...
byte b = 100;          // ✅ 100 is compile-time constant, fits in byte
...
char c = (char) x;     // ✅ Explicit cast needed
```
...
[⬅️ Back to Index](./README.md) | [Next: Operators ➡️](./09-operators.md)
