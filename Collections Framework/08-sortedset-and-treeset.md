...
> Note: For natural sorting order to work, the elements' class **must implement `Comparable`**. Wrapper classes and `String` already implement it. For custom classes, you must implement `Comparable` or provide an explicit `Comparator`.
...
[Previous: Set & HashSet](./07-set-and-hashset.md) · [Back to Collections Index](./README.md) · [Next: Comparable & Comparator](./09-comparable-and-comparator.md)
