...
Actual Cloning:
A a1 = new A();
A a2 = (A) a1.clone(); // valid New separate object created in heap with same field values!
```
...
### `Cloneable` ek Marker Interface hai
...
> Note: **Crucial Rule:** Agar class `Cloneable` implement **nahi** karti aur aap `clone()` call karte ho, toh runtime par **`CloneNotSupportedException`** throw hoga!
...
Original Object (a1) ──┐
                       ├──> Shared Inner Object (b1)  (Danger of side effects!)
Cloned Object (a2)   ──┘
```
...
public class Demo {
    public static void main(String[] args) throws CloneNotSupportedException {
        ...
        System.out.println(p1.address.city); //  Mumbai! (Original also changed!)
...
public class DemoDeep {
    public static void main(String[] args) throws CloneNotSupportedException {
        ...
        System.out.println(p1.address.city); // valid Delhi (Original remains safe!)
        System.out.println(p2.address.city); // Mumbai
    }
}
```

### ⚖ Comparison Table: Shallow vs Deep Cloning
...
### Example: Inspecting All Declared Methods
...
[Previous: Garbage Collection](./07-garbage-collection.md) · [Back to OOP Index](./README.md) · [Next: Singleton Pattern](./09-singleton-pattern.md)
