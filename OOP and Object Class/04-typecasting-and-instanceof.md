...
A a1 = new B();    // valid Upcasting — implicit
```
...
Employee e = (Employee) o;     // valid explicit downcast

System.out.println(e.id);      // valid now accessible
```
...
```java
A a1 = new A();
B b1 = (B) a1;    // valid compiles → [Error] ClassCastException at runtime
```
...
```java
A a = new B();
B b = (B) a;       // valid actual object is B
